...
def setup_connections(console: Console):
    ...
    try:
        ...
        # Yield 连接给调用者使用
        yield uav_clients, vrpn_clients
...
def run():
    ...
    - 无特殊情况处理（VRPN 是否启用不影响主循环）
    """
    console = Console()

    # 使用上下文管理器自动管理资源
    with setup_connections(console) as (uav_clients, vrpn_clients):
        # 显示频率配置信息
